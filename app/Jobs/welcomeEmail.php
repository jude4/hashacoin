...
class welcomeEmail implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $to = $user->email;
        $set = Settings::first();
        $subject = 'Welcome to '.$set->site_name;
        $mlogo = Logo::first();
        $from = env('MAIL_USERNAME');
        $logo = url('/') . '/asset/' . $mlogo->dark;
        $text = str_replace("{{logo}}", $logo, (str_replace("{{site_name}}", $set->site_name, str_replace("{{message}}", $set->welcome_message, $set->email_template))));
