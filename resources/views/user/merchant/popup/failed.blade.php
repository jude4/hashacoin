    ...
    <div class="card card-profile bg-white border-0 mb-5">
      <div class="card-body">
        <div class="text-center text-dark mb-5">
          <div class="btn-wrapper text-center mb-5 mt-5">
            <a href="javascript:void;" class="mb-3">
              <span class=""><i class="fal fa-sad-tear fa-6x text-success"></i></span>
            </a>
          </div>
          @if($link->status == 2)
          <h2 class="font-weight-bolder">{{__('Payment failed')}}</h2>
          <p class="text-dark mb-3">Payment of {{$link->getCurrency->real->currency_symbol.number_format($link->amount, 2)}} to {{$link->business()->name}} failed</p>
          @elseif($link->status == 0 && $link->payment_type == 'bank')
          <h2 class="font-weight-bolder">{{__('Payment pending')}}</h2>
          <p class="text-dark mb-3">Payment might be pending due to bank service. Please allow up to 2 hours for the settling bank to return a successful or failed transaction</p>
          @endif
          @if($url!=null)
        ...
        <div class="row justify-content-center">
          <a href="javascript:void;" data-href="{{$back}}" id="previous" class="btn btn-neutral btn-block mt-3">{{__('Try again')}}</a>
        </div>
