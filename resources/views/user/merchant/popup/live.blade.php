...
<div class="row justify-content-center mt-6">
    <div class="col-md-4 col-12">
        <div class="bhoechie-tab-container rounded mb-3">
            <div class="row">
                <div class="col-md-12 bhoechie-tab">
                    @if($link->getCurrency->card==1 && $link->user->business()->card==1)
                    <div class="bhoechie-tab-content rounded @if($type==null) active @else @if($type=='card') active @endif @endif rounded">
                        <div class="">
                            ...
                            <div class="card-body">
                                <form action="{{route('payment.submit', ['id'=>$link->ref_id])}}" method="post" id="payment-form">
                                    ...
                                    <div class="text-center">
                                        <button type="submit" id="payment-submit" class="btn btn-neutral btn-block my-4">
                                            {{__('Pay')}}
                    ...
                    <div class="bhoechie-tab-content rounded @if($type=='bank_account') active @endif">
                        ...
                        <div class="card-body">
                            <form action="{{route('payment.submit', ['id'=>$link->ref_id])}}" method="post" id="payment-form1">
                                ...
                                <input type="hidden" value="2" name="crf">
                                <div class="text-center">
                                    <button type="submit" id="ggglogin1" class="btn btn-neutral btn-block my-4">
                                        {{__('Pay')}}
                                        @if($link->user->business()->charges==1)
                                        {{$link->getCurrency->real->currency}}<span id="tcr">{{number_format(($link->amount*$link->getCurrency->percent_charge/100)+($link->getCurrency->fiat_charge)+($link->amount),2)}}</span>
                                        @else
                                    ...
                                    <div class="form-group row">
                                        <div class="col-md-12 mb-2">
                                            <div class="input-group">
                                                ...
                                                <input class="form-control @if ($errors->has('mobile'))is-invalid @endif" type="text" maxlength="12" name="mobile" autocomplete="off" value="@if(session('mobile')){{session('mobile')}}@endif" required>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="text-center">
                                        <button type="submit" id="ggglogin2" class="btn btn-neutral btn-block my-4">
                                            {{__('Pay')}}
                                            @if($link->user->business()->charges==1)
                                            {{$link->getCurrency->real->currency}}<span id="tcr">{{number_format(($link->amount*$link->getCurrency->percent_charge/100)+($link->getCurrency->fiat_charge)+($link->amount),2)}}</span>
                                            @else
                ...
                <div class="col-md-12 text-center">
                    <p class="text-xs font-weight-bold mb-3"><i class="fal fa-lock"></i> {{__('Secured by')}} <span class="fw-bold">{{$set->site_name}}</span></p>
                </div>
            </div>
        </div>
